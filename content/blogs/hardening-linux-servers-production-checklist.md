...
Hardening is only real if you can *prove* it. After applying the configuration above, these are the commands I run to validate the control set — the output below is sample engagement evidence.

### Effective SSH Configuration

```bash
$ sudo sshd -T | grep -Ei 'permitrootlogin|passwordauthentication|kexalgorithms|ciphers|maxauthtries|allowusers'
permitrootlogin no
passwordauthentication no
maxauthtries 3
kexalgorithms curve25519-sha256@libssh.org,diffie-hellman-group16-sha512
...
`sshd -T` dumps the *effective* merged config — the only reliable way to confirm a directive actually applied.

### SSH Server Grade (ssh-audit)

```bash
$ ssh-audit localhost:2222
  # algorithm recommendations (hardened)
  (kex)   curve25519-sha256@libssh.org   [pass]
  (cipher) chacha20-poly1305@openssh.com [pass]
  (mac)   hmac-sha2-512-etm@openssh.com  [pass]
...
$ lynis audit system --quick
  ...
  Hardening index : 85 [##########........]  (target >= 90)
  Warnings: 1
  Suggestions: 4
    - Configure password aging (login.defs)        [AUTH-9286]
    - Set restrictive umask 027                    [FILE-7524]
    - Disable unused filesystems (cramfs, freevxfs) [KRNL-6000]
```
...
```bash
$ sudo auditctl -s
AUDIT_STATUS: enabled 1, failure 1, pid 842, rate_limit 0, backlog_limit 8192
...
```bash
$ sudo rkhunter --check --skip-application-check --report-warnings-only
[ OK ] Rootkit checks
[ OK ] Suspicious file properties
[ WARNING ] SHORT_INODES (known false positive on overlayfs/container storage)
```

A hardening index of **85+** with a clean rkhunter pass is the bar I hold production builds to before they leave the lab.
